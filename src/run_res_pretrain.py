...
from models import ResPretrain, _is_chinese_char
...
MODEL_CLASSES = {
    'res-pretrain': (BertConfig, ResPretrain, BertTokenizer),
}
...
def create_dataset(args, input_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        for line in f:
            word = line.strip()
            if len(word) == 1 and _is_chinese_char(ord(word)):
                dataset.append(word)
    assert '嗯' in dataset
    return dataset
...
def make_features(args, examples, tokenizer, batch_processor):
    ...
    batch = {}
    batch['input_ids'] = tokenizer.convert_tokens_to_ids(examples)
...
def train(args, model, tokenizer, batch_processor):
    ...
    if args.local_rank == -1:
        train_dataset = create_dataset(args, 'vocab.txt')
    else:
        total_dataset = create_dataset(args, 'vocab.txt')
        start_position = 0
...
def evaluate(args, model, tokenizer, batch_processor, prefix=""):
    eval_dataset = create_dataset(args, 'vocab.txt')
...
if __name__=='__main__':
    ...
    parser.add_argument("--data_dir", default="/data/dobby_ceph_ir/neutrali/pretrained_models/roberta-base-ch-for-csc", type=str,
